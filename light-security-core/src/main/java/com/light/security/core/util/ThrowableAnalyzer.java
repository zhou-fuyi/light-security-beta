...
public class ThrowableAnalyzer {
     ...
     *
     * {@link InvocationTargetException}实例的默认提取器。
     *
     * @see InvocationTargetException#getTargetException()
     */
    public static final ThrowableCauseExtractor INVOCATION_TARGET_EXTRACTOR = new ThrowableCauseExtractor() {
        @Override
        public Throwable extractCause(Throwable throwable) {
     ...
     *
     * 翻译:
     * 比较器根据类的层次关系对类进行升序排序。 如果两个类具有层次关系，则此比较器认为“较高”的类较大。<br>
     * 对于层次无关的类，将对它们的完全限定名称进行比较。
     */
     ...
     * <li>{@link #INVOCATION_TARGET_EXTRACTOR} for {@link InvocationTargetException}</li>
     * </ul><br>
     ...
     * InvocationTargetExceptions will be handled by {@link #INVOCATION_TARGET_EXTRACTOR}
     * while all other throwables are handled by {@link #DEFAULT_EXTRACTOR}.
     *
     *
     * 翻译:
     * 初始化<code> Throwable </ code>和之间的关联 <code> ThrowableCauseExtractor </ code>。
     * 默认实现执行 以下注册：
     * <ul>
     *  <li> {@ link #DEFAULT_EXTRACTOR} for {@link Throwable} </ li>
     *  <li> {@ link #INVOCATION_TARGET_EXTRACTOR} for {@link InvocationTargetException} </ li>
     * </ul> <br>
     * 鼓励重写此方法的子类调用super方法以执行默认注册。 他们可以将其他提取程序注册为需要。
     * <p>
     * 注意：针对特定类型注册的提取器适用于该类型<i>及其所有子类型</ i>。
     * 但是，提取器注册的更具体类型保证首先被解决。 所以在默认情况下
     * InvocationTargetExceptions将由{@link #INVOCATION_TARGET_EXTRACTOR}处理,
     * 而所有其他可抛物则由{@link #DEFAULT_EXTRACTOR}处理。
     *
     * @see #registerExtractor(Class, ThrowableCauseExtractor)
     */
    protected void initExtractorMap() {
        registerExtractor(InvocationTargetException.class, INVOCATION_TARGET_EXTRACTOR);
     ...
     *
     * 向<code>extractorMap</code>中插入数据
     *
     ...
     *
     * 翻译:
     * 返回一个数组，其中包含为其注册了提取程序的类。 类的顺序是为了解析匹配的提取程序而进行比较的顺序。
     *
     ...
     *
     * 翻译:
     * 确定提供的<code> Throwable </ code>的原因链。 返回的数组包含使用已注册的对象
     * 从stacktrace中提取的所有 throwables {@link ThrowableCauseExtractor}。
     * 数组的元素是有序的：第一个元素是传入的throwable本身。 以下元素按其顺序在堆栈跟踪中向下显示。
     * <p>
     * 注意：如果没有为此实例注册任何{@link ThrowableCauseExtractor}，则返回的数组将始终仅包含传入的throwable。
     *
    ...
    public final Throwable[] determineCauseChain(Throwable throwable){
        ...
        while (currentThrowable != null) {
            chain.add(currentThrowable);//这里保证了chain数据不为空, 不管是否能够提取到数据, 保证了原始数据存在于chain中(第一个元素是传入的throwable本身)
     ...
     *
     * 翻译:
     * 从传入的数组中返回可 转换（isInstance） 为提供的类型的第一个throwable。 返回的实例可以安全地强制转换为指定的类型。
     * <p>
     * 如果传入的数组为null或为空，则此方法返回<code> null </ code>。
    ...
    public final Throwable getFirstThrowableOfType(Class<? extends Throwable> throwableType, Throwable[] chain) {
        if (chain != null) {
            for (Throwable t : chain) {
                //isInstance 判断 t 是否可以转换为 throwableType
     ...
     * of the type itself). If <code>expectedBaseType</code> is <code>null</code>, no check
     * will be performed.
     *
     * 验证提供的throwable是提供的类型（或类型本身）的有效子类。 如果<code> expectedBaseType </ code>为<code> null </ code>，则不会执行任何检查。
     *
     * <p>
